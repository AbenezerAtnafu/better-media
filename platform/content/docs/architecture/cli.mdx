...
Add the framework (which includes the CLI binary) or the CLI package directly. **Pick your package manager** in the tab below—the same choice is kept for every command on this page.

<Tabs groupId="cli-package-manager" items={['pnpm', 'npx', 'yarn']} persist>
  <Tab value="pnpm">
```bash
...
<Tabs groupId="cli-package-manager" items={['pnpm', 'npx', 'yarn']}>
  <Tab value="pnpm">
```bash
